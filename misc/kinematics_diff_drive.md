...
---
## Matrix form
Lets express the inverse kinematic equation is matrix form as:
$$\overrightarrow{V}_{wheels} = \textcolor{red}{M}\ .\overrightarrow{\tau} $$
and, forward kinematic equation as:
$$  \overrightarrow{\tau} = \textcolor{red}{M^+}\ .\ \overrightarrow{V}_{wheels} $$
where,
$n$: number of wheels,
$\overrightarrow{V}_{wheels}$ is wheel velocity vector of size $n\ X\ 1\  $,
$\textcolor{red}{M}$ is kinematic matrix of size $n\ X\ 3\  $,
$\textcolor{red}{M^+}$ is pseudo-inverse kinematic matrix of size $3\ X\ n\  $,
$\overrightarrow{\tau}$ is twist vector of size $3\ X\ 1\  $
